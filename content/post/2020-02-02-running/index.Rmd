...
* Irrespective of a run's mileage, there was never a run that ended less than a tenth of mile from an integer. 
...
I was recently reminded of a work on how influential round numbers are in our efforts. For example, students are more likely to [retake the SAT](https://journals.sagepub.com/doi/abs/10.1177/0956797610391098){target="_blank"} when their scores fall just below a round number, and [marathon finising times](https://www.nber.org/papers/w20343){target="_blank"} tend to bunch just below 4 hours rather than above it. I decided to look for evidence of this phenomenon in my own running by looking at how much my mileage on each run deviated from an integer.

I treated each positive integer as a round number and calculated how much each run's mileage deviated from the closet integer.
...
run19 %>% 
  ...
  labs(x = "Miles Run", 
       ...
       subtitle = "The length of shorter runs (< 11 miles) tended to deviate more from integers than longer runs (>= 11 miles)") + 
  theme_classic() + 
  theme(axis.text = element_text(color = "black"), 
        legend.position = "none") + 
  annotate(geom = "text", x = 15, y = 0.19, 
           label = "Half marathons\n(13.10 miles each)",
           color = "#F57670") + 
  annotate(geom = "text", x = 26, y = 0.28, 
           label = "Full marathon\n(26.20 miles)", 
           color = "#649ED4") + 
  annotate(geom = "text", x = 13.2, y = -0.26, 
           label = "Hot weather,\ncut this run short", 
           color = "#1BB940") + 
  annotate(geom = "rect", xmin = 1, xmax = 27, ymin = -0.11, ymax = -0.01,
           alpha = .2) + 
  annotate(geom = "text", x = 5.5, y = -0.06, 
           label = "There was a never a run that ended\nless a tenth of a mile from an integer", 
           color = "black")
```
...
It's as if I was telling myself during my runs, consciously or not, "try to it something and a half miles" or "try to hit the next mile." But once I got just above it, it didn't matter anymore until I got close to the next round number.
